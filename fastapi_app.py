...
# คอลัมน์ที่เราจะส่งให้ frontend
PRODUCT_EXPORT_COLS = [
    "product_id",
    "category",
    "brand",
...
def get_product_share(product_row: pd.Series) -> float:
    ...
    stock_i = float(product_row.get("stock_quantity", 0.0))
    share = stock_i / total_stock

    # กันกรณี 0 หรือเล็กเกินไป
    if share <= 0:
        share = 1.0 / len(products_df)
    return share
...
def predict_item_stock(req: PredictItemStockRequest):
    """
    ผู้ใช้เลือกสินค้า + horizon (7/30 วัน) + current_stock (ถ้าไม่ใส่ ใช้จาก CSV)
    → ทำนาย demand ในอนาคตของสินค้านั้น และประมาณสต็อกคงเหลือหลังครบ horizon
    """
    # หา product
    ...
    if req.current_stock is not None:
        ...
        base_stock = float(product_row.get("stock_quantity", 0.0))

    horizon = int(req.horizon_days)
    if horizon <= 0:
        return {"success": False, "message": "horizon_days ต้องมากกว่า 0"}

    # ทำนาย demand รวมทั้งร้าน ล่วงหน้า horizon วัน
    try:
    ...
    item_daily_demand = [f * share_i for f in store_forecasts]
